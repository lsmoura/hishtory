...
func Encrypt(userSecret string, data, additionalData []byte) ([]byte, []byte, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to make AEAD: %w", err)
	}
	nonce := make([]byte, 12)
	if _, err := io.ReadFull(rand.Reader, nonce); err != nil {
		return nil, nil, fmt.Errorf("failed to read a nonce: %w", err)
	}
	ciphertext := aead.Seal(nil, nonce, data, additionalData)
	_, err = aead.Open(nil, nonce, ciphertext, additionalData)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to open AEAD: %w", err)
	}
